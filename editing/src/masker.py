...
# we make heuristics not to include these in the EDITOR.
stopwords = [
    'i', 'me', 'my', 'myself', 'we', 'our', 'ours', 'ourselves', 'you', 'your',
    'yours', 'yourself', 'yourselves', 'he', 'him', 'his', 'himself', 'she', 'her',
    'hers', 'herself', 'it', 'its', 'itself', 'they', 'them', 'their', 'theirs',
    'themselves', 'what', 'which', 'who', 'whom', 'this', 'that', 'these', 'those',
    'am', 'a', 'an', 'the', 'and', 'but', 'if', 'or', 'as', 'by', 'for',
    'with', 'about', 'against', 'between', 'into', 'through', 'from', 'up', 'down',
    'in', 'out', 'on', 'off', 'here', 'there', 'where', 'why', 'how', 'all', 'any',
    'both', 'each', 'few', 'more', 'most', 'other', 'some', 'no', 'nor', 'not', 'only',
    'own', 'same', 'so', 'than', 'too', 'very', 'n\'t', '\'s',
]
not_allowed_pos_list = [
    'PROPN',
    'NOUN',
    'PRON',
    'SYM',
...
class Masker():
    ...
    def get_all_masked_strings(self, editable_seg, previous_text=None):
        """
        ...
        for i, token in enumerate(editor_toks):
            ...
            masked_segs[i] = previous_text + editable_seg[:token_start] + Masker._get_sentinel_token(0) + editable_seg[token_end:]
            labels[i] = Masker._get_sentinel_token(0) + editable_seg[token_start:token_end] + Masker._get_sentinel_token(1)
...
class GradientMasker(Masker):
    ...
    def __init__(
        ...
        self.use_heuristic_masks = use_heuristic_masks
    ...
    def _get_gradients_by_prob(self, instance, pred_idx):
        ...
        embedding_gradients = []
        hooks = self.predictor._register_embedding_gradient_hooks(embedding_gradients)
        
        dataset = Batch(instances)
        dataset.index_instances(self.predictor._model.vocab)
        dataset_tensor_dict = util.move_to_device(dataset.as_tensor_dict(), self.predictor.cuda_device)
    ...
    def _get_word_positions(self, predic_tok, editor_toks):
        ...
        if predic_tok_end > editor_toks[return_word_idx].idx_end:
            ...
            if last_idx != len(editor_toks) - 1 and predic_tok_end > editor_toks[last_idx].idx_end:
                raise MaskError

            return return_indices, return_starts, return_ends

        return_tuple = ([return_word_idx], [editor_toks[return_word_idx].idx], [editor_toks[return_word_idx].idx_end])
        return return_tuple
    ...
    def is_allowed(self, predic_tok):
        """Determine positions that are not allowed to be masked. If `self.use_heuristic_masks` is False,
        then just exclude special tokens like '<|endoftext|>' for GPT2. Otherwise, also consider POS and
        stopword exclusion criteria.
        """
        not_special_tok = predic_tok not in self.predictor_special_toks
        if not self.use_heuristic_masks:
            return not_special_tok
        else:
            correct_pos = predic_tok.pos_ not in not_allowed_pos_list
            not_named_ent = predic_tok.ent_type_ not in not_allowed_named_ents
            not_stopword = predic_tok.tag_ == False
            return not_special_tok and correct_pos and not_stopword and not_named_ent
    ...
    def get_important_editor_tokens(
        ...
        if labeled_instance is None:
            labeled_instance = self.predictor.json_to_labeled_instances({"sentence": editable_seg})[0]

        grad_type_options = [
            "integrated_l1", "integrated_signed", "normal_l1", "normal_signed", "normal_l2", "integrated_l2"
        ]
        ...
        if predic_tok_end_idx is not None:
            if predic_tok_start_idx is not None:
                grad_magnitudes = grad_magnitudes[predic_tok_start_idx:predic_tok_end_idx]
                grad_signed = grad_signed[predic_tok_start_idx:predic_tok_end_idx]
            else:
        ...
        ordered_word_indices_by_grad = [
            self._get_word_positions(all_predic_toks[idx], editor_toks)[0]
                for idx in ordered_predic_tok_indices
                # filter based on rules:
                if self.is_allowed(all_predic_toks[idx])
        ]
