...
class Editor():
    def __init__(
        ...
        if ints_to_labels is None:
            ints_to_labels = get_ints_to_labels()
    ...
    def input_to_instance(self, inp, editable_seg=None, return_tuple=False):
        ...
        if editable_seg is None:
            instance = self.dataset_reader.text_to_instance(inp, device=self.device)
        else:
            instance = self.dataset_reader.text_to_instance(editable_seg, device=self.device)
        if return_tuple:
    ...
    def get_sorted_token_indices(self, inp, grad_pred_idx):
        ...
        `grad_pred_idx` is the label towards which we will get gradients (either original label or the contrast label).
        """
    ...
    def get_candidates(
        ...
        num_spans, token_ind_to_mask, masked_inp, orig_spans, max_length = self._prepare_input_for_editor(
    ...
    def _process_gen(self, masked_inp, gen, sentinel_toks):
        ...
        for idx, (token, sentinel_tok) in enumerate(tok_sentinel_iterator):
            if len(sentinel_tok) == 12:
                sentinel_idx = sentinel_tok[-2:-1]
            else:
                sentinel_idx = sentinel_tok[-3:-1]
    ...
    def _get_pred_with_replacement(self, temp_gen, orig_spans, *args):
        ...
        temp_instance = self.dataset_reader.text_to_instance(temp_gen, device=self.device)
    ...
    def _sample_edits(
        ...
        while editable_segs != []:
            ...
            for inp_idx, (editor_input, editable_seg, masked_token_ids,
                ...
                masked_token_ids_tensor = torch.LongTensor(masked_token_ids + [self.tokenizer.eos_token_id]
                        ).unsqueeze(0).to(self.device)
                eos_id = self.tokenizer.eos_token_id
                bad_tokens_ids = [[x] for x in range(sentinel_start, end_token_id)] + [[eos_id]]
                max_length = max(int(4/3 * max_length), 200)
                logging.info(wrap_text("Sub round: " + str(num_sub_rounds)))
                logging.info(wrap_text(f"Input: {inp_idx} of {num_inputs-1}"))
                logging.info(wrap_text(f"Last sentinel: {last_sentin}"))
                logging.info(wrap_text("INPUT TO EDITOR: " + f"{self.tokenizer.decode(masked_token_ids)}"))
                ...
                logging.info(wrap_text("first batch: " + batch_decoded[0]))
                for batch_idx, batch in enumerate(batch_decoded):
                    sentinel_toks = [f"<extra_id_{idx}>" for idx in range(0, span_end + 1)]
                    bad_gen, first_bad_tok, temp, stripped_batch = self._process_gen(editable_seg, batch, sentinel_toks)
                    ...
                    if bad_gen:
                        ...
                        new_editable_token_ids = np.array(self.tokenizer.encode(temp)[:-1])
                        ...
                        new_orig_token_ids = np.array(
                            orig_token_ids[np.where(orig_token_ids == new_first_token)[0][0]:]
                        )
                        sentinel_indices = np.where(
        ...
        for idx, es in enumerate(edited_editable_segs):
            ...
            assert "<extra_id_" not in es, f"Extra id token should not be in edited inp: {es}"
            assert "</s>" not in es, f"</s> should not be in edited inp: {edited_editable_segs[idx][0]}"
