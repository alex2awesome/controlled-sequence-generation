...
from utils_general import format_layer_freezes, freeze_all_params, get_config, reshape_and_pad_sequence, \
    vec_or_nones, get_device
import re
...
class BaseDiscriminator(nn.Module):
    ...
    def forward(self, input_ids=None, add_features=None, labels=None, attention_mask=None,
        ...
        if isinstance(input_ids, list):
            ...
            # this branch is hit during IG (and unperturbed/baseline generation to score). Let's just return y_pred_ll and see if it'll work
            loss, y_pred_ll, _ = self.predict_one_doc(
                input_ids=input_ids,
                labels=labels,
                attention_mask=attention_mask,
                ...
                label_idx=kwargs.get('label_idx'),
                generate=kwargs.get('generate')
            )
            if (input_ids is not None and len(input_ids) > 1) or (inputs_embeds is not None and len(inputs_embeds) > 1):
                y_pred_ll = y_pred_ll.squeeze()
            return y_pred_ll # [:, 0] # todo: temp to see if this is the last blocker for IG
...
class Discriminator(LightningMixin, BaseDiscriminator):
    ...
    def _predict_one_doc_one_pass(self, input_ids, labels=None, add_features=None,
