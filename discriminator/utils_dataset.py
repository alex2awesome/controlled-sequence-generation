...
from util.utils_general import reformat_model_path, get_idx2class, _get_attention_mask, format_local_vars, \
    transpose_dict, get_fh
    ...
    def setup(self, stage=None):
        ...
        if stage in ('fit', None):
            ...
            # split by filename
                ...
                train_dataset = list(filter(lambda x: x[2] in ['train', 'val'], zip(d.X, d.y, d.split, d.v)))
                train_X, train_y, _, add_features = zip(*train_dataset)
                self.train_dataset = Dataset(X=train_X, y=train_y, v=add_features)
                test_dataset = list(filter(lambda x: x[2] in ['test'], zip(d.X, d.y, d.split, d.v)))
