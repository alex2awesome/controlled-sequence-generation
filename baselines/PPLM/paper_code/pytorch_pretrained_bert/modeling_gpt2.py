...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    Outputs:
        if `lm_labels` is not `None`:
            ...
            `lm_logits`: the language generator_pplm logits as a torch.FloatTensor of size [batch_size, sequence_length, config.vocab_size]
...
class GPT2DoubleHeadsModel(GPT2PreTrainedModel):
    ...
    Outputs:
        if `lm_labels` and `multiple_choice_labels` are not `None`:
            ...
            `lm_logits`: the language generator_pplm logits as a torch.FloatTensor of size [batch_size, num_choices, sequence_length, config.vocab_size]
