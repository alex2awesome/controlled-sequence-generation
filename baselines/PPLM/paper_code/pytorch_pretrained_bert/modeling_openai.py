...
class OpenAIGPTLMHead(nn.Module):
    ...
    def forward(self, hidden_state):
        # Truncated Language generator_pplm logits (we remove the last token)
...
class OpenAIGPTLMHeadModel(OpenAIGPTPreTrainedModel):
    ...
    Inputs:
        ...
        `token_type_ids`: an optional torch.LongTensor with the same shape as input_ids
            ...
            The input, position and token_type embeddings are summed inside the Transformer before the first
            self-attention block.
        `lm_labels`: optional language generator_pplm labels: torch.LongTensor of shape [batch_size, sequence_length]
            with indices selected in [-1, 0, ..., vocab_size]. All labels set to -1 are ignored (masked), the loss
            is only computed for the labels set in [0, ..., vocab_size]

    Outputs:
        if `lm_labels` is not `None`:
            Outputs the language generator_pplm loss.
        else:
            `lm_logits`: the language generator_pplm logits as a torch.FloatTensor of size [batch_size, sequence_length, total_tokens_embeddings]
...
class OpenAIGPTDoubleHeadsModel(OpenAIGPTPreTrainedModel):
    ...
    Inputs:
        ...
        `token_type_ids`: an optional torch.LongTensor with the same shape as input_ids
            ...
            The input, position and token_type embeddings are summed inside the Transformer before the first
            self-attention block.
        `lm_labels`: optional language generator_pplm labels: torch.LongTensor of shape [batch_size, num_choices, sequence_length]
    ...
    Outputs:
        if `lm_labels` and `multiple_choice_labels` are not `None`:
            Outputs a tuple of losses with the language generator_pplm loss and the multiple choice loss.
        else: a tuple with
            `lm_logits`: the language generator_pplm logits as a torch.FloatTensor of size [batch_size, num_choices, sequence_length, total_tokens_embeddings]
