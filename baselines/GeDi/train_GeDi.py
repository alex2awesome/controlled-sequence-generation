...
import baselines.GeDi.util as u
...
def train(args, train_dataset, model, tokenizer):
    ...
    if os.path.exists(args.model_name_or_path) and isinstance(args.model_name_or_path.split("-")[-1].split("/")[0], int):
    ...
    for epoch_ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.mask_eos_token:
                ...
                if args.add_sep:
                    # adding the sep token would require extending the loss mask of ones by one position
                    # to the right (equivalent to prepending one on the left)
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    all_token_type_ids = []
    for f in features:
        if f.token_type_ids is None:
            all_token_type_ids.append(np.zeros(len(f.input_ids)))
        else:
...
def main():
    ...
    if args.env == 'bb':
        ...
        import zipfile
        fs.get('aspangher/transformer-pretrained-models/%s.zip' % args.model_name_or_path, '%s.zip' % args.model_name_or_path)
        with zipfile.ZipFile('%s.zip' % args.model_name_or_path, 'r') as zip_ref:
            zip_ref.extractall()
        ...
        remote_dir = os.path.join('aspangher', 'controlled-sequence-gen', args.data_dir)
        fs.get(remote_dir, args.data_dir, recursive=True)

    # Setup distant debugging if needed
    if (args.server_ip != 'none') and (args.server_port != 'none'):
    ...
    config_class, model_class, tokenizer_class = u.MODEL_CLASSES[args.model_type]
    handle_name = lambda x: x if args.env != 'bb' else './' + x

    config = config_class.from_pretrained(
        handle_name(args.config_name) if args.config_name else handle_name(args.model_name_or_path),
        num_labels=num_labels,
        finetuning_task=args.task_name,
        cache_dir=args.cache_dir if (args.cache_dir != 'none') else None,
    )
    if args.outbias:
        if args.threeway:
            config.nbias = 3
    ...
    tokenizer = tokenizer_class.from_pretrained(
        handle_name(args.tokenizer_name) if args.tokenizer_name else handle_name(args.model_name_or_path),
    ...
    model = model_class.from_pretrained(
        handle_name(args.model_name_or_path),
    ...
    if args.env == 'bb':
        fs.put(args.output_dir, args.remote_dir.replace('s3://', ''))
